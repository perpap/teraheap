...
#include <math.h>
#include "utilities/growableArray.hpp"  // For GrowableArray
#include "utilities/pair.hpp"

Pair<size_t, size_t> calculate_regions(size_t total_regions, int active_workers, size_t cur_region, int worker_id){
	size_t regions_per_worker = total_regions / active_workers;
	size_t extra_regions = total_regions % active_workers; // For uneven division

	size_t start_region = cur_region + worker_id * regions_per_worker;
	size_t end_region = start_region + regions_per_worker;

	if (worker_id < static_cast<int>(extra_regions)) {
	    // If the worker has an ID less than extra_regions, that worker will handle one additional region.
	    // Adjust the start_region and end_region to account for the extra regions.
	    start_region += worker_id;
	    end_region = start_region + regions_per_worker + 1;
	} else {
	    start_region += extra_regions;
	    end_region = start_region + regions_per_worker;
	}
...
#if 1//perpap
#ifdef TERA_MAJOR_GC
  class ParallelCompactH2CompleteTransfersTask: public AbstractGangTask {
	  public:
          ParallelCompactH2CompleteTransfersTask(const char* name)
          : AbstractGangTask(name) {
	  }

          virtual void work(uint worker_id){
	      Universe::teraHeap()->h2_complete_transfers(worker_id);
	  }
  };
  ...
  public:
	  ...
	  virtual void work(uint worker_id){
		  ...
		  //if(regions.first != regions.second)
		  {
			  ...
			  while (beg_bit < range_end) {
				  ...
				  if (Universe::teraHeap()->get_policy()->h2_transfer_policy(obj)) {
					  ...
					  */
					  ...
					  PSParallelCompact::_mark_bitmap.unmark_h2_candidate_obj(obj);
				  }
			  ...
			  PSParallelCompact::_summary_data._region_data[region_idx].set_forwarding_table(fd_table);
			  assert(PSParallelCompact::_summary_data._region_data[region_idx].data_size() <= region_size,
					  "Region size differ, size before = %lu and size after = %lu", region_size, PSParallelCompact::_summary_data._region_data[region_idx].data_size());
...
void ParallelCompactData::compact_h2_candidate_objects(HeapWord* source_beg,
  ...
  while (cur_region < end_region) {
    ...
    while (beg_bit < range_end) {
      ...
      //if (Universe::teraHeap()->is_in_h2(h2_addr)) {
        // Enable grouping of objects
        Universe::teraHeap()->enable_groups(h1_addr, h2_addr, 0);//FIXME added: 0 for single thread
        cm->update_contents(obj);
        Universe::teraHeap()->disable_groups(0);//FIXME added: 0 for single thread

        // Move objects to H2
        Universe::teraHeap()->h2_move_obj(h1_addr, h2_addr, size, 0);//FIXME added: 0 for single thread
...
#ifdef TERA_TIMERS
  Universe::teraHeap()->getTeraTimer()->h2_clear_fwd_table_start();//FIXME TeraTimers
  //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H2_CLEAR_FWD_TABLE");
#endif
  // Get the start region the space
  size_t cur_region = addr_to_region_idx(source_beg);
  // Get the last region of the top pointer of the region
...
#ifdef TERA_TIMERS
  Universe::teraHeap()->getTeraTimer()->h2_clear_fwd_table_end();//FIXME TeraTimers
#endif
}
...
HeapWord* ParallelCompactData::calc_new_pointer(HeapWord* addr, ParCompactionManager* cm, uint gc_thread_id) const {//FIXME added: gc_thread_id
  ...
  if (EnableTeraHeap && PSParallelCompact::mark_bitmap()->is_h2_marked(addr)) {
    ...
    return Universe::teraHeap()->compact_h2_candidate_obj_enabled(cm->gc_thread_id()) ? 
...
#ifdef TERA_TIMERS
  Universe::teraHeap()->getTeraTimer()->h2_precompact_start();//FIXME TeraTimers
  //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H2_PRECOMPACT");
#endif //TERA_TIMERS
#if 1//perpap
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_summary_phase_start();//FIXME TeraTimers
    //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H1_SUMMARY_PHASE");
#endif //TERA_TIMERS
...

  {
    ...
    if (EnableTeraHeap) {
      ...
      if(EnableParallelH2Compact){
          ParallelCompactH2CompleteTransfersTask task("ParallelCompactH2CompleteTransfersTask");
          ParallelScavengeHeap::heap()->workers().run_task(&task); 
      }
      else{	    
...
#ifdef TERA_TIMERS
      Universe::teraHeap()->getTeraTimer()->h2_mark_bwd_ref_start();//FIXME TeraTimers
      //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H2_MARKING_BWD_REF");
#endif //TERA_TIMERS
...
#ifdef TERA_TIMERS
      Universe::teraHeap()->getTeraTimer()->h2_mark_bwd_ref_end();//FIXME TeraTimers
#endif //TERA_TIMERS
    }
#endif //TERA_MAJOR_GC
...
#ifdef TERA_TIMERS
   //CREATE_SCOPED_TIMER_IF(EnableTeraHeap, h1_marking_phase_timer, std::chrono::milliseconds, "H1_MARKING_PHASE");
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_marking_phase_start();//FIXME TeraTimers
    //TeraTimers::createScopedTimer<>("H1_MARKING_PHASE");
    //Universe::teraHeap()->getTeraTimer()->createScopedTimer<>("H1_MARKING_PHASE");
#endif //TERA_TIMERS
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_marking_phase_end();//FIXME TeraTimers
#endif //TERA_TIMERS
}
...
public:
  ...
  void work(uint worker_id) {
    ParCompactionManager* cm = ParCompactionManager::gc_thread_compaction_manager(worker_id);
    PCAdjustPointerClosure adjust(cm, worker_id);//FIXME added: worker_id
    {
      ResourceMark rm;
...
void PSParallelCompact::compact_h2_candidate_objects() {
  ...
  //Universe::teraHeap()->getTeraTimer()->h2_total_flush_buffer_nofreespace_elapsed_time();
  //Universe::teraHeap()->getTeraTimer()->h2_total_async_request_elapsed_time();
  #endif
...
#ifdef TERA_TIMERS
  Universe::teraHeap()->getTeraTimer()->h2_adjust_bwd_ref_start();//FIXME TeraTimers
  //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H2_ADJUST_BWD_REF");
#endif

  ParCompactionManager* cm = ParCompactionManager::get_vmthread_cm();
  oop *obj = Universe::teraHeap()->h2_adjust_next_back_reference();
  uint gc_id = EnableParallelH2Compact ? ParallelGCThreads : 0;//FIXME
  while (obj != NULL) {
    Universe::teraHeap()->enable_groups(NULL, (HeapWord*) obj, gc_id);//FIXME added: ParallelGCThreads
    adjust_pointer(obj, cm, gc_id);//FIXME added: ParallelGCThreads
    Universe::teraHeap()->disable_groups(gc_id);//FIXME added: ParallelGCThreads
    obj = Universe::teraHeap()->h2_adjust_next_back_reference();
  }

#ifdef TERA_TIMERS
  Universe::teraHeap()->getTeraTimer()->h2_adjust_bwd_ref_end();//FIXME TeraTimers
#endif
}
#endif //TERA_MAJOR_GC
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_adjust_roots_start();//FIXME TeraTimers
    //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H1_ADJUST_ROOTS");
#endif
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_adjust_roots_end();//FIXME TeraTimers
#endif
}
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_compact_start();//FIXME TeraTimers
    //Universe::teraHeap()->getTeraTimer()->createScopedTimer("H1_COMPACT");
#endif
...
#ifdef TERA_TIMERS
  if (EnableTeraHeap)
    Universe::teraHeap()->getTeraTimer()->h1_compact_end();//FIXME TeraTimers
#endif
}
...
MoveAndUpdateClosure::do_addr(HeapWord* addr, size_t words) {
  ...
  assert(PSParallelCompact::summary_data().calc_new_pointer(source(), compaction_manager(), compaction_manager()->gc_thread_id() ) ==
