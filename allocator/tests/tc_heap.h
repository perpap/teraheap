...
#include <sys/mman.h>
#include <limits.h>
#define CARD_SIZE ((uint64_t) (1 << 13))
#define PAGE_SIZE sysconf(_SC_PAGESIZE)

#define GB ((uint64_t)1 << 30) //1GB
#define CONVERT_TO_GB(bytes) bytes >> 30
#define H1_SIZE 64 * GB
#define H1_ALIGNMENT PAGE_SIZE
#define H2_SIZE 512 * GB //512GB
#define H2_ALIGNMENT CARD_SIZE * PAGE_SIZE
...
void print_heap_statistics(){
    printf("h1 mapped at: (HEX)%p (DEC)%zd\n", h1.start_address, (uintptr_t)h1.start_address);
    ...
    printf("h1_end is aligned? %d\n", is_aligned(align_ptr_up((void *)h1_end, h2.alignment), h2.alignment));
    // Init allocator
    //init(H2_ALIGNMENT, "/spare2/perpap/fmap/", H2_SIZE, (void *)h1_end);
    printf("h2 mapped at: (HEX)%p (DEC)%zd\n", h2.start_address, (uintptr_t)h2.start_address);
    printf("%-20s %-20s %-20s %-20s %-20s %-20s\n", "", "START_ADDRESS(HEX)", "START_ADDRESS(DEC)", "END_ADDRESS(HEX)", "END_ADDRESS(DEC)", "SIZE(GB)");
    printf("%-20s %-20p %-20llu %-20p %-20llu %-20td\n", "H1", h1.start_address, (long long unsigned int)h1.start_address, (void *)h1_end, (long long unsigned int)h1_end,CONVERT_TO_GB((ptrdiff_t)((char *)h1_end-h1.start_address)));
    printf("%-20s %-20p %-20llu %-20p %-20llu %-20td\n", "H2", h2.start_address, (long long unsigned int)h2.start_address, h2.end_address, (long long unsigned int)h2.end_address,CONVERT_TO_GB((ptrdiff_t)(h2.end_address-h2.start_address)));
}
