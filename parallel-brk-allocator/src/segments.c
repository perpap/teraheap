...
struct id_mapping *id_mappings_array = NULL;
...
void init_regions(uint32_t gc_threads, const char *h2_write_policy) {
  ...
  */
  ...
  id_mappings_array = (struct id_mapping*) malloc(( MAX_PARTITIONS * max_rdd_id) * sizeof(struct id_mapping));
  check_allocation_failure(id_mappings_array, "[Error] - Failed to allocate memory for id_mappings_array\n"); 
...
*/
  ...
  for (int32_t i = 0; i < rdd_id_size; i++) {
    id_mappings_array[i].mapped_region = NULL;

    /* Mapping Lock */
    if (mtx_init(&id_mappings_array[i].index_lock, mtx_plain) != thrd_success) {
     #if DEBUG_PRINT
      fprintf(allocator_log_fp, "[%s|%s|%d] Index lock initialization failed! error:%d\n", __FILE__, __func__, __LINE__, thrd_error);
      #endif
      exit(EXIT_FAILURE);
    }
  }
  ...
  /* New Region Lock */
  if (mtx_init(&new_region_lock, mtx_plain) != thrd_success) {
    #if DEBUG_PRINT
    fprintf(allocator_log_fp, "[%s|%s|%d] New region lock initialization failed! error:%d\n", __FILE__, __func__, __LINE__, thrd_error);
    #endif
    exit(EXIT_FAILURE);
  }
}
...
char* allocate_to_region(size_t size, uint64_t rdd_id, uint64_t partition_id) {
  ...
  struct region *mapped_region = NULL;
  int32_t id_index = get_id(rdd_id, partition_id);

  mtx_lock(&id_mappings_array[id_index].index_lock);

  if (id_mappings_array[id_index].mapped_region == NULL) {
    mtx_lock(&new_region_lock);

    char* res = new_region(size);

    mtx_unlock(&new_region_lock);
    ...
    if (size < (uint64_t) REGION_SIZE) {
      id_mappings_array[id_index].mapped_region =
          &region_array[((res + size) - region_array[0].start_address) / ((uint64_t)REGION_SIZE)];
      id_mappings_array[id_index].mapped_region->rdd_id = rdd_id;
      id_mappings_array[id_index].mapped_region->part_id = partition_id;
    }
    ...
    mtx_unlock(&id_mappings_array[id_index].index_lock);

    return res;
  }

  mapped_region = id_mappings_array[id_index].mapped_region;

  if (mapped_region->last_allocated_end + size > mapped_region->start_address + (uint64_t)REGION_SIZE) {
    mtx_lock(&new_region_lock);
    ...
    /* If object spans more than 1 region we don't want to allocate more objects with it*/
    if (size < (uint64_t) REGION_SIZE){
      id_mappings_array[id_index].mapped_region = &region_array[(res+size - region_array[0].start_address) / (uint64_t)REGION_SIZE];
      id_mappings_array[id_index].mapped_region->rdd_id = rdd_id;
      id_mappings_array[id_index].mapped_region->part_id = partition_id;
    }
  ...
  mapped_region->last_allocated_start = mapped_region->last_allocated_end;
  mapped_region->last_allocated_end = mapped_region->last_allocated_start + size;
...
#if DEBUG_PRINT
  printf("Allocating from region %ld until region %ld\n",(mapped_region->last_allocated_start - region_array[0].start_address) / (uint64_t)REGION_SIZE, (mapped_region->last_allocated_start+size - region_array[0].start_address) / (uint64_t)REGION_SIZE);
#endif

  char* res = mapped_region->last_allocated_start;
...
struct region_list* free_regions() {
  ...
  for (i = 0; i < region_array_size; i++){
    if (region_array[i].used == 0 && region_array[i].last_allocated_end != region_array[i].start_address){
      ...
      if (id_mappings_array[get_id(region_array[i].rdd_id, region_array[i].part_id)].mapped_region == &region_array[i]) {
        id_mappings_array[get_id(region_array[i].rdd_id, region_array[i].part_id)].mapped_region = NULL;
      }
...
char* get_region_start_addr(char *obj, uint64_t rdd_id, uint64_t part_id) {
	...
	return id_mappings_array[index].mapped_region->start_address;
}
