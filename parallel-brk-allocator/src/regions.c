...
struct _mem_pool {
  ...
  _Atomic uint64_t size;    // Current allocated bytes in TeraCache
};
...
uint64_t _GC_THREADS = 0;
char **top_alloc_per_thread = NULL;
...
void init(bool is_parallel, uint64_t gc_threads, const char *h2_write_policy, uint64_t align, const char* path, uint64_t size, char* h1_end) {
  ...
  top_alloc_per_thread = malloc(gc_threads * sizeof(char *));

  // if (top_alloc_per_thread == NULL) {
  //   fprintf(stderr, "Malloc failed\n");
  //   exit(EXIT_FAILURE);
  // }

  for (int i = 0; i < gc_threads; i++) {
    top_alloc_per_thread[i] = tc_mem_pool.cur_alloc_ptr;
  }
...
void* reserve_aligned_anonymus(size_t size, size_t alignment) {
  // Alignment must be a multiple of page size
  size_t pagesz = (size_t) sysconf(_SC_PAGESIZE);
  if (alignment < pagesz || (alignment % pagesz) != 0) {
    errno = EINVAL;
    return MAP_FAILED;
  }

  size_t request = size + alignment;
  void *p = mmap(NULL, request, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
  ...
  // Unmap the leading and trailing slivers to leave exactly [aligned, aligned+size)
  size_t lead = (size_t)(aligned - addr);
  size_t trail = (size_t)((addr + request) - (aligned + size));

  if (lead && munmap((void *) addr, lead) != 0) {
    munmap((void *) addr, request);
    return MAP_FAILED;
  }

  if (trail && munmap((void *)(aligned + size), trail) != 0) {
...
char* allocate(size_t size, uint64_t rdd_id, uint64_t partition_id, uint64_t gc_thread) {

  if (gc_thread < 0 || gc_thread >= _GC_THREADS) {
    fprintf(stderr, "Gc thread (%lu - %lu)out-of-bounds!\n", gc_thread, _GC_THREADS);
    exit(EXIT_FAILURE);
  }

	char* alloc_ptr = NULL;
  char* cur_alloc_ptr = NULL;
  ...
  cur_alloc_ptr = (char *) (((uint64_t) alloc_ptr) + size * HEAPWORD);

	if (top_alloc_per_thread[gc_thread] < cur_alloc_ptr) {}
		top_alloc_per_thread[gc_thread] = cur_alloc_ptr;

	// Alighn to 8 words the pointer (TODO: CHANGE TO ASSERTION)
	if ((uint64_t) top_alloc_per_thread[gc_thread] % HEAPWORD != 0)
		top_alloc_per_thread[gc_thread] = (char *)((((uint64_t)top_alloc_per_thread[gc_thread]) + (HEAPWORD - 1)) & -HEAPWORD);
...
void update_top(void) {
  char *current_top = tc_mem_pool.cur_alloc_ptr;

  for (int i = 0; i < _GC_THREADS; i++) {
    if (top_alloc_per_thread[i] > current_top) {
      current_top = top_alloc_per_thread[i];
    }
  }

  for (int i = 0; i < _GC_THREADS; i++) {
    top_alloc_per_thread[i] = current_top;
  }

  tc_mem_pool.cur_alloc_ptr = current_top;
}
...
int	r_areq_completed() {
  ...
  fprintf(stderr, "r_areq_completed called\n");
  return 1;
}
...
int	r_areq_completed_parallel(uint32_t worker_id) {
  ...
  fprintf(stderr, "r_areq_completed_parallel called\n");
  return 1;
}
