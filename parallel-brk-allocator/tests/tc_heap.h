...
void initialize_h1(uint64_t alignment, const char *mount_point, uint64_t size, char *address){
  ...
  // h1.start_address = (char *) aligned_mmap(size, alignment, NULL, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS | MAP_NORESERVE, -1, 0);
  h1.start_address = (char *) reserve_aligned_anonymus(size, alignment);
