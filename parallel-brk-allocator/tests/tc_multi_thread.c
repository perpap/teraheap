...
void *multi_alloc(void *arg) {
  ...
  for (int i = 0; i < 10; i++) {
    if ((0 < thread_id && thread_id < 4) || (8 < thread_id && thread_id < 12)) 
      objects[thread_id * 10 + i] = allocate(SIZE_1MB, 0, 0, thread_id);
    else
      objects[thread_id * 10 + i] = allocate(SIZE_1MB, thread_id+1, 0, thread_id);
...
int main(int argc, char **argv) {
  if(argc != 4){
    fprintf(stderr,"Usage: ./tc_multi_thread.bin <mount_point> <h1_size> <h2_size>\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < 10 * THREADS - 1; i++) {
    for (int j = i + 1; j < 10 * THREADS; j++){
      if (objects[i] == objects[j]) {
        printf("TC_Multi_Thread: Double Allocation [%d] == [%d] == %p\t\t\t\033[1;31m[FAIL]\033[0m\n", i, j, objects[i]);
        free(objects);
        return 1;
      }
    }
  ...
  if (cur_alloc_ptr() != calculated_top + 8 * SIZE_1MB) {
    printf("TC_Multi_Thread: Top address incorrect [%p] == [%p]\t\t\t\033[1;31m[FAIL]\033[0m\n", cur_alloc_ptr(), calculated_top + 8 * SIZE_1MB);
    free(objects);
    return 1;
  }
  ...
  printf("--------------------------------------\n");
  printf("TC_Multi_Thread:\t\t\t\033[1;32m[PASS]\033[0m\n");
  printf("--------------------------------------\n");
